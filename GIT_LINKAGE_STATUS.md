...
SPEC_AGENT (r6): probe GREEN — §7b creds-prerequisite retired (confirmed: sidecar carries git creds on sce-coalition; 403 was a different deployment). §2/§3 schema corrected: git/branches response is paginated {data:{items:[{name}]}} — NOT {branches:[]}; branch items carry only {name} (no commitId); HEAD comes from git/commits[0] not branches listing; projectDefaultBranch unreliable on this cluster — use mainRepository.defaultRef.value (authoritative). §3 helpers (get_branch_head, list_commits, project_default_branch) updated to reflect verified shapes. §4 sharp-edges note updated. get_checkpoint_for_commit marked ⚠ UNRESOLVED in §2 + §3 (probe ran inline, body not captured; gates Phase 3 only). IMPLEMENTING_AGENT action: no code changes needed for Phase 1/2 — shapes already fixed in app.py commit 13c6840; this spec now matches.
