...
### D45: Unknown Assignee Resolution via Global Users Fetch
**Decision**: After loading bundles and collaborators, the app detects assignees that have an ID but empty `name` field and are not found in the project collaborators cache. For these, it fetches the global users list (`GET /v4/users`) once and resolves names by matching IDs. Resolved users are patched into both the bundle data and the `projectMembersCache` so dropdowns work correctly. Additionally, the assignee column in the table tries a userName-based fallback match when the ID lookup fails (governance vs v4 API ID mismatch).
**Rationale**: The Domino governance API sometimes returns `stageAssignee: {id: "xxx", name: ""}` with an empty name — the user exists but the governance API doesn't populate the name field. The v4 collaborators endpoint only returns project members, not all users, so a user who is assigned but no longer a collaborator won't appear there either. The global users list is the only reliable source for resolving these orphaned assignee IDs. This adds one conditional API call (only when unknowns exist) but eliminates "Unknown user" labels in the UI.
